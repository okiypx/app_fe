 ...
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 获取的元素，查找不到时반환null,如果参数不合法，直接반환参数.
 */
 ...
 * @param {Object} [thisObject] 函数调用时的this指针，如果没有此参数，默认예当前遍历的数组
 * @remark
...
/**
 * opera 从10스타트不예用opera后面的字符串进行版本的判断
 * 在Browser identification最后추가Version + 数字进行版本标识
 * opera后面的数字保持在9.80不变
 */
 ...
 * @param {string} position 插入html的位置정보，取值为beforeBegin,afterBegin,beforeEnd,afterEnd
...
/**
 * 운영flash对象的方法，包括创建flash对象、获取flash对象以及判断flash插件的版本号
 * @namespace baidu.swf
 */
baidu.swf = baidu.swf || {};
 ...
 * @param {String} 	options.errorMessage 		未安装flash player或flash player版本号过低时的제시
 ...
 * @param {boolean} options.menu 				예아니显示右键菜单，允许值：true/false
 * @param {boolean} options.loop 				播放到最后一帧时예아니重新播放，允许值： true/false
 * @param {boolean} options.play 				flash예아니在浏览器加载时就스타트播放。允许值：true/false
 * @param {string} 	options.quality 			设置flash播放的画质，允许值：low/medium/high/autolow/autohigh/best
 * @param {string} 	options.scale 				设置flash함유량如何缩放来适应设置的宽高。允许值：showall/noborder/exactfit
 ...
 * @param {boolean} options.allowfullscreen 	예아니允许flash全屏。允许值：true/false
 * @param {boolean} options.seamlesstabbing 	允许设置执行아니缝跳格，从而使사용자能跳出flash应用程序。该参数只能在安装Flash7及更高版本的Windows中使用。允许值：true/false
 * @param {boolean} options.devicefont 			设置静态文本对象예아니以设备字体呈现。允许值：true/false
 * @param {boolean} options.swliveconnect 		第一次加载flash时浏览器예아니应启动Java。允许值：true/false
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
         ...
         * @param {Function} [callBack] 异步调用后将반환值作为参数的调用回调函数，如아니반환值，可以不传入此参数
         * @return {Null}
        */
 ...
 * @config {String} vars.picDescFieldName POST请求中图片기술的key,默认值'picDesc'
 * @config {Number} vars.maxSize 文件的최고体积,单位'MB'
 * @config {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @config {Number} vars.maxNum:32 최고上传多少个文件
 * @config {Number} vars.compressLength 能接受的최고边长，超过该值会等比压缩
 * @config {String} vars.url 上传的url地址
 * @config {Number} vars.mode mode == 0时，예使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @param {String|Function} selectFileCallback 문서 선택的回调
 * @param {String|Function} exceedFileCallback文件超出限制的최고体积时的回调
 * @param {String|Function} deleteFileCallback 삭제文件的回调
 * @param {String|Function} startUploadCallback 스타트上传某个文件时的回调
 * @param {String|Function} uploadCompleteCallback 某个文件上传完成的回调
 * @param {String|Function} uploadErrorCallback 某个文件上传실패的回调
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * 삭제文件
     * @public
     * @param {Number|Array} [index] 要삭제的index，不传则全部삭제
...
/**
 * 삭제script的属性，再삭제script标签，以解决修复内存泄漏的问题
 * 
 * @param {HTMLElement} src script节点
 */
...
/**
 * 으로script标签加载数据，加载完成由浏览器端触发回调
 * @name baidu.sio.callByBrowser
 * @function
 * @grammar baidu.sio.callByBrowser(url, opt_callback, opt_options)
 * @param {string} url 加载数据的url
 * @param {Function|string} opt_callback 数据加载종료时调用的函数或函数名
 * @param {Object} opt_options 다른可选项
 * @config {String} [charset] script的字符集
 * @config {Integer} [timeOut] 超时시각，超过这个시각将不再响应本请求，并触发onfailure函数
 * @config {Function} [onfailure] timeOut设定后才生效，到达超时시각时触发本函数
 * @remark
 * 1、与callByServer不同，callback参数只支持Function类型，不支持string。
 * 2、如果请求了一个不存在的页面，callback函数在IE/opera下也会被调用，因此使用者需要在onsuccess函数中判断数据예아니正确加载。
 * @meta standard
 ...
 * @config {Integer} [timeOut] 超时시각(单位：ms)，超过这个시각将不再响应本请求，并触发onfailure函数
 * @config {Function} [onfailure] timeOut设定后才生效，到达超时시각时触发本函数
 * @remark
...
baidu.sio.callByServer = /**@function*/function(url, callback, opt_options) {
    ...
    /*
     * 반환一个函数，用于立即（挂在window上）或者超时（挂在setTimeout中）时执行
     */
...
baidu.json.parse = function (data) {
    //2010/12/09：更新至不使用原生parse，不检测사용자输入예아니正确
...
/**
 * 将字符串解析成json对象，为过时상호 작용，今后会被baidu.json.parse代替
...
/**
 * 将json对象序列化，为过时상호 작용，今后会被baidu.json.stringify代替
