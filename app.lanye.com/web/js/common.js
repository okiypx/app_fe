...
(function($){
	$().ready(function(){
	    ...
	    $(".addcar").click(function(event){
	        ...
	        flyer.fly({ 
	            start: { 
	                left: event.pageX, //스타트位置（必填）#fly元素会被设置成position: absolute
	                top: event.pageY //스타트位置（必填）
	            }, 
	            end: { 
	                left: offset.left+50, //종료位置（必填） 
	                top: offset.top, //종료位置（必填） 
	                width: 0, //종료时宽度 
	                height: 0 //종료时高度 
...
//$(function () {
//	var options = {
//		nav_menuClass: ".nav",//一级导航的class이름
//		secMenu: ["#list2", "#list3", "#list4", "#list5", "#list6", "#list7"],   //下拉菜单的id 이름 要예多个下拉的话就在后面加（, "#list3"）
//		index: [1, 2, 3, 4, 5, 6]             //导航上的第几个菜单下面有下拉菜单 ，注释：从0스타트计算的，要예多个下拉的话就在后面加（,4）
//	}
//	new InitNav(options);
//})
...
//					var _field = _wrap.find('li:first'); //此变量不可放置于函数起始处,li:first取值예变化的
//					var _h = _field.height(); //取得每次滚动高度(多行滚动情况下,此变量不可置于스타트处,아니则会有间隔时长延时)
//					_field.animate({
//							marginTop: -_h + 'px'
//						},
//						600,
//						function () { //으로取负margin值,隐藏第一行
//							_field.css('marginTop', 0).appendTo(_wrap); //隐藏后,将该行的margin值置零,并插入到最后,实现아니缝滚动
//						})
//				},
//				_interval) //滚动间隔시각取决于_interval
...
var SlideAnimateCollection = {
	...
	"fade":function(scope){
		scope.sliderLis.has(":visible").fadeOut(600);//括号为毫秒数  渐变淡出시각
		scope.sliderLis.eq(scope.current).fadeIn(600);//渐变淡入시각
	},
